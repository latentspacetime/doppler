...
from doppler import estimate_recall, simulate_captures
...
def row(rates: dict[str, float], heterogeneity: float) -> tuple[float, float, float]:
...
def main() -> None:
    for label, rates in (("2 sources", RATES), ("3 sources", THIRD_SOURCE)):
        ...
        for heterogeneity in HETEROGENEITY:
            truth, estimated, coverage = row(rates, heterogeneity)
            print(
